# params: SYSTEM_HOST, INFO_FLAG_LIMIT, SUBMIT_FLAG_LIMIT, TIMEZONE
import datetime
...
from models import FlagStatus, FlagSubmit, Flag
...
class API:
    def __init__(self, host: str, timezone: str, version='v1'):
        self.api_base = f'{host}/api/flag/{version}'
    ...
    def parse_flag_info_response(self, flag: str, response):
        if response.status_code == 200:
            info = response.json()
            if self.flag_is_fresh(info):
                return True, None
            return False, FlagSubmit(flag, FlagStatus.REJECTED, 'expired')
        ...
        if respcode == GetInfoResult.ERROR_RATELIMIT:
            return False, FlagSubmit(flag, FlagStatus.QUEUED, 'flag info ratelimit')

        return False, FlagSubmit(
            flag,
            FlagStatus.QUEUED,
            f'error response from flag getinfo: {respcode}'
        )

    def info_flags(self, *flags: str):
        responses = (requests.get(f'{self.api_base}/info/{flag}') for flag in flags)
        return dict(zip(flags, (self.parse_flag_info_response(i, j) for i, j in zip(flags, responses))))

    @staticmethod
    def parse_flag_submit_response(flag: str, response):
        try:
        ...
        for status, possible_codes in RESPONSES.items():
            if result_code in possible_codes:
                return FlagSubmit(flag, status, RESPONSES[status][result_code])

        return FlagSubmit(
            flag,
            FlagStatus.QUEUED,
            f'unknown checksystem code: {result_code}'
        )
...
def submit_flags(flags: list[Flag], config):
    ...
    info_rate = int(config['INFO_FLAG_LIMIT'])
    submit_rate = int(config['SUBMIT_FLAG_LIMIT'])
    ...
    queued = map(
        lambda f: FlagSubmit(f, FlagStatus.QUEUED, 'flag submission ratelimit'),
